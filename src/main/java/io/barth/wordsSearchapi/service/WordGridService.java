package io.barth.wordsSearchapi.service;
...
public class WordGridService {
    ...
    public  char[][] generateGrid(int gridSize, List<String> words){
        ...
        for(String word: words){
            ...
            for(Coordinate coordinate: coordinates){
                int x = coordinate.x;
                int y = coordinate.y;
                Direction selectedDirection = getFittedDirection(contents, word, coordinate);
    ...
    private Direction getFittedDirection(char[][] contents, String word, Coordinate coordinate){
        List<Direction> directions = Arrays.asList(Direction.values());
        Collections.shuffle(directions);
        for(Direction direction: directions){
            if (doesItFit(contents, word, coordinate, direction)) {
                return direction;
            }
        }
        return null;
    }
    private boolean doesItFit(char[][] contents, String word,Coordinate coordinate, Direction direction){
        int gridSize = contents[0].length;
    ...
    private void fillEmptyGrid(char[][] contents){
        int gridSize = contents[0].length;
